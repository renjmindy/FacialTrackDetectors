...
class batch_norm(object):
    ...
    def __call__(self, x, train=True):
        #return tf.compat.v1.contrib.layers.batch_norm(x, decay=self.momentum, updates_collections=None, epsilon=self.epsilon,
        #                                    center=True, scale=True, is_training=train, scope=self.name)
        return tf.compat.v1.layers.batch_normalization(x, momentum=self.momentum, epsilon=self.epsilon, center=True, 
